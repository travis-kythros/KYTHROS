...
function jobCustomerSearch(q) {
  ...
  const matches = val.length === 0
    ...
    : allCustomers.filter(c => (c.name||'').toLowerCase().includes(val)).slice(0, 10);

  if (matches.length === 0 && val.length > 0) {
    drop.innerHTML = `<div style="padding:10px 14px;font-size:.82rem;color:var(--muted);font-style:italic">No match — will create as new customer</div>`;
  ...
  drop.innerHTML = matches.map(c => {
    const phone = c.phone ? `<span style="color:var(--muted);font-size:.75rem;margin-left:6px">${esc(c.phone)}</span>` : '';
    return `<div onclick="jobPickCustomer('${c.id}')" style="padding:9px 14px;cursor:pointer;border-bottom:1px solid var(--line);font-size:.84rem;transition:background .1s" onmouseover="this.style.background='rgba(217,119,6,.1)'" onmouseout="this.style.background=''">${esc(c.name||'')}${phone}</div>`;
  }).join('');
  drop.style.display = 'block';
}
...
window.jobCustomerSearch = jobCustomerSearch;
window.jobPickCustomer = jobPickCustomer;
